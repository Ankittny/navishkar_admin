...
use App\Models\Order as OrderCancel;
...
class OrderController extends BaseController
{
  	...
  	public function return_cancel(Request $request)
    {
        ...
        $waybill_no = $orderDetail->third_party_delivery_tracking_id;
        if($request->delivery_status == 'canceled'){
            $this->cancelWaybill2($request);
        }
        if($request->delivery_status == 'returned'){
            $this->delevery_returned($request);
        }
        if ($orderDetail) {
            $orderDetail->delivery_status = $request->delivery_status;
            $orderDetail->save();
            return response()->json([
                'success' => true,
  	...
  	public function printslip(Request $request)
    {
        $client = new Client();
        try {
            $response = $client->get('https://track.delhivery.com/api/p/packing_slip', [
                'query' => [
                    'wbns' => $request->input('third_party_delivery_tracking_id'),
                    'pdf' => 'true'
                ],
                'headers' => [
                    'Content-Type' => 'application/json',
            ...
            return response()->json([
                'error' => 'An error occurred while fetching the packing slip.',
  	...
  	public function hnbinvoice(Request $request, $order_id, $detail_id){
        ...
        $companyName = getWebConfig(name: 'company_name');
        $companyWebLogo = getWebConfig(name: 'company_web_logo');
        $order = $this->orderRepo->getFirstWhere(params: ['id' => $order_id], relations: ['seller', 'shipping', 'details']);
        $vendor = $this->vendorRepo->getFirstWhere(params: ['id' => $order['details']->first()->seller_id]);
        $invoiceSettings = json_decode(json: $this->businessSettingRepo->getFirstWhere(params: ['type'=>'invoice_settings'])?->value);
        $mpdf_view = PdfView::make(Order::HNB_INVOICE[VIEW],
        compact('order', 'vendor', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo','invoiceSettings','detail_id')
        );
        $this->generatePdf(view: $mpdf_view, filePrefix: 'order_invoice_',filePostfix: $order_id, pdfType: 'invoice');
    }
}
