                                            ...
                                            <td>
                                                <div class="media align-items-center gap-10">
                                                    ...
                                                    <div>
                                                        ...
                                                        @endif
                                                      @if(!empty($detail->third_party_delivery_tracking_id) && !empty($detail->delivery_service_name))
                                                      		<div>
                                                                <strong>
                                                   ...
                                                   <div>
                                                      ...
                                                      <form action="{{ route('admin.orders.hnbinvoice', ['order_id' => $detail['order_id'], 'primary_id' => $detail['id']]) }}" method="POST" style="display:inline;">
                                                            @csrf
                                                            <button type="submit" class="btn btn-link download-hnbinvoice-slip">
                                                                <p>Print H&B Invoice</p>
                                                            </button>
                                                        </form>
                                                      @endif
                                                    </div>
...
<script>
    $(document).ready(function() {
        $('.orderAction_delivery_status').on('change', function() {
            var selectedValue = $(this).val();
            var dataId = $(this).data('id');
            const selectElement = document.querySelector('.orderAction_delivery_status');
            const third_party_delivery_tracking_id = selectElement.getAttribute('data-third_party_delivery_tracking_id-id');
            $.ajax({
                url: "{{ route('admin.orders.return-cancel') }}", // Ensure there are no extra spaces
                type: "POST",
                data: { delivery_status: selectedValue , dataId:dataId, third_party_delivery_tracking_id:third_party_delivery_tracking_id  },
                headers: {
 ...
 function myFunction(id) {
    ...
    const third_party_delivery_tracking_id = id;

    // Send the AJAX request
    $.ajax({
        url: "{{ route('admin.orders.printslip') }}",
        type: 'POST',
        contentType: 'application/json',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Fetching CSRF token
        },
        data: JSON.stringify({
            third_party_delivery_tracking_id: third_party_delivery_tracking_id
        }),
        success: function(response) {
            // Check if response is valid and has the expected structure
            if (response && response.packages && response.packages.length > 0) {
                const downloadLink = response.packages[0].pdf_download_link;

                // Ensure download link exists
                if (downloadLink) {
                    const a = document.createElement('a');
                    a.href = downloadLink;
                    a.download = 'packing_slip.pdf'; // Filename for the downloaded file
                    document.body.appendChild(a);
                    a.click();
                    document.body.removeChild(a); // Clean up the DOM
                } else {
                    console.error('Download link is missing in the response.');
                }
            } else {
